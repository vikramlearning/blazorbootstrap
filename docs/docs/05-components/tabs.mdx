...
| ShowTabByIndexAsync(int tabIndex) | Selects the tab by index and show its associated pane. |
| ShowTabByNameAsync(string tabName) | Selects the tab by name and show its associated pane. |
...
| Name | string | Gets or sets the tab name. | | |
...
[See demo here.](https://demos.blazorbootstrap.com/tabs#return-object-on-tab-switch)
